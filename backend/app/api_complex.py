...
async def set_dify_config(
    config: DifyConfigCreate, db: Session = Depends(get_db)
):
...
async def get_dify_config(db: Session = Depends(get_db)):
    ...
    if not config:
        raise HTTPException(
            status_code=404, detail="Dify configuration not found"
        )
...
async def upload_document(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calling Dify API: {e}"
        )
...
async def chat(
    ...
    return StreamingResponse(
        generate_dify_response(), media_type="text/event-stream"
    )
